...
function safeFileName(original) {
    let name = original.replace(/[^\u4e00-\u9fa5\w\.-]/g, "").replace(/\s+/g, "_");
    if (!name) name = "bg_" + getRandomNum();
    const ext = name.split('.').pop();
    const main = name.slice(0, -(ext.length + 1)) || "bg";
    return { main, ext };
}
...
async function uploadFile() {
    ...
    const { main, ext } = safeFileName(file.name);
    let newName = `${main}.${ext}`;

    // 防重名：自动加随机数字
    while (fileList.includes(newName)) {
        newName = `${main}_${getRandomNum()}.${ext}`;
    }

    const fd = new FormData();
    const renamedFile = new File([file], newName, { type: file.type });
    fd.append("ulfile", renamedFile);
    fd.append("upload", "上传");
